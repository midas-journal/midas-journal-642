...
*/
  ...
  itkSetMacro( State, bool );
  itkGetConstReferenceMacro( State, bool );
  ...
  void SetLevelSet( const IdCellType& i, InputImagePointer Im )
  {
    ...
    ImageRegionIterator< InputImageType > in ( Im,
  ...
  void SetFunctionCount( const IdCellType& n )
  {
    ...
    IdCellType k = 1;
    for( typename std::vector< IdCellType >::iterator it = m_Lookup.begin();
      it != m_Lookup.end(); ++it, ++k )
        *it = k;
  }
