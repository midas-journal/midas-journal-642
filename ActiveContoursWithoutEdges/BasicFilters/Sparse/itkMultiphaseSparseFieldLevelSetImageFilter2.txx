...
double MultiphaseSparseFieldLevelSetImageFilter<TInputImage, TOutputImage, TFunction >
::m_ConstantGradientValue = 1.0;

template<class TInputImage, class TOutputImage, class TFunction >
const ITK_TYPENAME MultiphaseSparseFieldLevelSetImageFilter<TInputImage,
TOutputImage, TFunction >::ValueType
MultiphaseSparseFieldLevelSetImageFilter<TInputImage, TOutputImage, TFunction >
...
template<class TInputImage, class TOutputImage, class TFunction >
const ITK_TYPENAME MultiphaseSparseFieldLevelSetImageFilter< TInputImage,
TOutputImage, TFunction >::StatusType
MultiphaseSparseFieldLevelSetImageFilter< TInputImage, TOutputImage, TFunction >
...
::Initialize()
{
	for ( unsigned int functionIndex = 0; functionIndex < this->m_FunctionCount;
		functionIndex++ )
	{
	...
	for ( unsigned int j = 0; j < this->m_FunctionCount; j++ )
	{
...
::PostProcessOutput()
{
  ...
  for ( unsigned int i = 0; i < this->m_FunctionCount; i++ )
  {
    ...
    for ( inIt.Begin(), outIt.Begin(); !outIt.IsAtEnd(); ++inIt, ++outIt )
      if ( inIt.Get() < 0 )
        outIt.Value() =  p;
  }
}
